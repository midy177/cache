...
func TestIncrementOverflowInt(t *testing.T) {
	...
	x, _ := tc.Get("i")
	if x.(int8) != -128 {
		t.Error("i did not overflow as expected; value:", x.(uint8))
	}
...
func TestIncrementOverflowUint(t *testing.T) {
	...
	x, _ := tc.Get("uint8")
	if x.(uint8) != 0 {
		t.Error("uint8 did not overflow as expected; value:", x.(uint8))
	}
}
...
func TestDecrementUnderflowUint(t *testing.T) {
	...
	x, _ := tc.Get("uint8")
	if x.(uint8) != 255 {
		t.Error("uint8 did not underflow as expected; value:", x.(uint8))
	}
}
